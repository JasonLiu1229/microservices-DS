...
@router.put("/{invite_id}/status/{status}/user/{user_id}")
def update_invite_status(invite_id: int, status: str, user_id: int) -> None:
    """
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            wrapper.update_invitation(invitation_id=invite_id, status=status, user_id=user_id)
...
def create_invite(invite: Invite) -> None:
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            wrapper.create_invitation(user_id=invite.user_id, event_id=invite.event_id, Invitee_id=invite.Invitee_id)
            return Response(status_code=200)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) from e
